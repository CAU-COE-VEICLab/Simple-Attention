...
def load_checkpoint(config, model, optimizer, logger):
...
def save_checkpoint(config, epoch, model, max_miou, optimizer, logger):
    ...
    save_path = os.path.join(config.save_dir, f'ckpt_epoch_{epoch}.pth')
    logger.info(f"{save_path} saving......")
    torch.save(save_state, save_path)
    logger.info(f"{save_path} saved !!!")


def save_checkpoint_guorun(config, epoch, model, max_miou, optimizer, logger):
    save_state = {'model': model.state_dict(),
                  'optimizer': optimizer.state_dict(),
                  'max_miou': max_miou,
                  'epoch': epoch,
    ...
    save_path = os.path.join(config.save_dir, f'ckpt_epoch_best_guorun.pth')
    logger.info(f"{save_path} saving......")
    torch.save(save_state, save_path)
    logger.info(f"{save_path} saved !!!")
...
def add_mask_to_source_multi_classes(source_np, mask_np, num_classes):
    colors = [[0, 0, 0], [0, 0, 255], [255, 255, 0], [0, 255, 0], [255, 0, 255], [0, 255, 255], [255, 255, 0]]
